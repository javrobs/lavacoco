...
const Orden = () => {
    ...
    const [others,setOthers] = useState([...others_start,{concept:"",qty:"",price:""}]);
    const [othersTinto,setOthersTinto] = useState(others_tinto);
    const [totals,setTotals] = useState({total:"",totalTinto:""})
    ...
    const SubMenuButton = ({icon,buttonCategory,classNameExtra}) => {
        return <button className={`flex gap-1 ${classNameExtra} items-center !shadow-none !rounded-none justify-center btn-white h-14 ${buttonCategory==category?"choice":""}`} onClick={()=>setCategory(buttonCategory)}>{prices[buttonCategory].name}<Icon icon={icon}/></button>
    }
    ...
    function sendFinish(){
        ...
        if(formRef.current.reportValidity()){
            ...
            fetcher(`/api/set_and_promote_order_list/${order.id}`);
        }
    }
            ...
            <div className="bubble-div p-4">
                ...
                <form ref={formRef} autoComplete="off" className="max-w-screen-md mt-3 mx-auto flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                        ...
                        <button type="button" className="flex items-center gap-1 btn-go" onClick={()=>{setAreYouSure(true)}}>Guardar y terminar<Icon icon="shopping_cart_checkout"/></button>
                    </div>
                    <ModalWarning show={areYouSure} warningMessage="Después de este punto, no podrás modificar la orden más">
                        <button className="btn btn-back" type="button" onClick={(e)=>{e.preventDefault();setAreYouSure(false)}}>No, regresar<Icon icon="undo"/></button>
                        <button onClick={sendFinish} className="btn btn-go" type="button" name="save-and-promote" >Sí, continuar<Icon icon="check_circle"/></button>
                    </ModalWarning>
                </form>:<>
                <div className="grid rounded-lg shadow-sm overflow-hidden grid-cols-2 sm:grid-cols-4 mt-3">
                    <SubMenuButton classNameExtra="border-b-[1px] sm:border-b-0 border-e-[1px] border-slate-300" buttonCategory={1} icon="local_laundry_service"/>
                    <SubMenuButton classNameExtra="border-b-[1px] border-s-[1px] sm:border-b-0 sm:border-x-[1px] border-slate-300" buttonCategory={2} icon="iron"/>
                    <SubMenuButton classNameExtra="border-t-[1px] border-e-[1px] sm:border-t-0 sm:border-x-[1px] border-slate-300" buttonCategory={3} icon="dry_cleaning"/>
                    <SubMenuButton classNameExtra="border-t-[1px] sm:border-t-0 border-s-[1px] border-slate-300" buttonCategory={4} icon="king_bed"/>
                </div>
